...
> **2026-08-26 补充(0.4.2,轨迹借力)**:发现官方 `@deepseek-ai/dsh-client-ui-trajectory` 已是成熟的**事件台账视图**(conversationViews + conversation.view Tab + 事件详情面板 + loadOlder 分页)。结论:不是重复造轮子——版本/回退/分叉仍是官方没有的;会重复的是三处**线性形态**(线性消息/思考时间线、事件详情查看、自绘浮层视图基建)。本次修订:时间线迁移为官方视图 Tab、详情让位官方轨迹、跳转适配视图切换;**保留 P2 分叉图/分支拓扑展示层**(消息/思考节点的分支呈现,官方无分支可视化,2026-08-26 修正)。详见 §5.1/§5.2/§10。
...
- **节点内容**:版本节点(操作类型图标、时间、`markerText` 摘要、产物变更徽标、git 头哈希);版本列表内不做线性消息/思考节点行(那是官方轨迹的台账形态)。
- **不做的是「线性台账式消息/思考时间线」**:按时间一维展开的消息/思考/工具列表 = 官方轨迹已覆盖(thinking quote / assistant content / tool call / 折叠 / 耗时 / token)——那种形态再做即纯重复(2026-08-26 决定)。
- **保留的是「分叉图/分支拓扑」(P2,展示层)**:见 §5.2——把消息/思考节点放进**分支/流程图**呈现(撤回/编辑点分叉、旧路径 vs 新路径),官方没有任何分支可视化,这是补"怎么走到这里"的维度,不是重复。

### 5.2 分叉图/思考流(P2)——消息/思考节点的分支拓扑展示层

> 定位修正(2026-08-26):分叉图**不是**官方轨迹的重复——官方轨迹是**线性事件台账**(单一记录路径,看"发生了什么");分叉图是**分支拓扑展示层**(看"怎么走到这里的"):在撤回/编辑/恢复点分叉,消息/思考节点作为分支卡片,展示旧路径(被回退的阴影区间)与新路径的演化关系。官方 UI 只有子代理层级链接,没有分叉/分支可视化——这是我们的差异化展示层。
...
- **数据面借官方(2026-08-26 决定)**:分叉 = 会话树,数据面用官方 `sessions` 服务、`fork` API、subagent 目录,不自建平行数据;视图壳复用官方 `conversation.view` 机制(注册第二个 Tab,如「分叉」);消息/思考节点原文与摘要直接读官方轨迹同源数据(thinking quote / assistant content / tool call),不搞第二套渲染管线。
- **节点**:回合(输入→思考→输出→工具链折叠);边:推进;分叉点:标记节点,显示"此处回退:旧路径(阴影区间) vs 新路径"。
- **渲染**:SVG 连线 + 小图元卡片;节点摘要 + 点击跳转对话(复用 5.1 机制);大图虚拟化。
- **价值(复盘/审计)**:回答"对话是怎么一步步走到这里的"——被回退的旧路径与当前路径并列可见,是时间线(变更史)的拓扑补充,也是复盘/审计视图(P2.3)的底座。
...
### P2 — 分叉图 + 增强(旗舰)
- [~] 2.1 分叉流程图(骨架完成,2026-08-26):数据面 `retrace/forkmap` 投影单元(镜像 foldSurface + 边界 replacedSeqs,不截断)+ `GET /forkmap` 降级;视图面「分叉」Tab(脊柱 + 旧路径卡片 + 跳转);SVG 图形化/节点原文/聚合留 2.2 与后续。顺带修复 0.4.2 程序化切视图静默 no-op(tab-bar DOM click)
- [ ] 2.2 分支意图卡:回退区间摘要 + 产物列表 + 思考概要(LLM 摘要缓存)
- [ ] 2.3 精选增强:版本对比(对话/文件 diff)、保存点、审计视图
- 明确不做:线性台账式消息/思考时间线(官方轨迹已覆盖,§5.1)
...
| **0.4.2** | ✅ 已发(2026-08-26):轨迹借力 P0(时间线迁移官方视图 Tab + 详情让位轨迹 + 跳转适配视图切换) | 对齐官方机制 |
| **0.4.5** | ✅ 已发(2026-08-28):P2.1 分叉图骨架 + 稳定性批(per-marker guard/原输入残留/压缩消息隐藏操作入口/review 批) | 分叉图骨架 + 基础体验修复 |
| 0.5.x | P2 完成:分叉图 + 意图卡 + 精选增强 | 旗舰功能上线 |

> 每个 P 完成后发布并写 CHANGELOG + DEVLOG;README 同步新品牌与功能矩阵。
> 旧包 `dsh-message-editor` 的 npm deprecate 可随任一后续版本发布(0.2.2 冻结,README 已含迁移指引)。
