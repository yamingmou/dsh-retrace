...
>
> **两道判据都成立才武装**（2026-09-18 第二轮）：宿主那条只是"不反对"，客户端还有**一票否决** ——
> 本页 `navigator.userAgent` 含 `Electron`，或页面 URL 含 `dsh-desktop-`（外部报告那套壳的查询参数
> 前缀）⇒ 一律不武装。宿主侧的判据也补成四条请求级证据：能力头 `x-dsh-desktop-renderer` /
> 请求 UA 含 Electron / 请求 URL **或 `Referer`** 含 `dsh-desktop-`（我们自己的轮询 URL 没有 query，
> 所以页面标记实际是从 `Referer` 读到的），**任一成立**即判桌面页面。这样"壳什么桌面痕迹
> 都不给"的宿主（报告人那台：纯 Node 跑 harness + Electron 渲染页，三条旧判据全不成立，
> 旧实现把"完全没有证据"归成了浏览器页 ⇒ 照旧武装 ⇒ 仍退不掉）也不会再把退出卡住。
> 桌面端退不掉时，把设置里的「退出确认（关闭守卫）」关掉即可立即恢复（不用重启）。
