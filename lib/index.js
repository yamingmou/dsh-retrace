...
import { attachCloseGuard, runningSessions, sessionRunningState, guardSurfaceOf } from './close-guard.js'
    ...
    const disposers = [
      ...
      harness.handle('retrace.runningState', async (args) => {
        try {
          ...
          // guardSurfaceOf(ctx, undefined) 只能靠宿主痕迹判:桌面宿主 → unknown
          // (客户端不武装原生门,安全侧),web 宿主 → browser。
          return { ok: true, value: { running: runningSessions(ctx), ...guardSurfaceOf(ctx, undefined) } }
        } catch (error) {
          return { ok: false, error: { code: 'internal', message: String(error) } }
        }
      }),
