...
/** 取（必要时建）权威短码解析器;失败返回 null（调用方自行兜底）。 */
function shortcodeResolverOrNull() {
  try {
...
function canonicalBadge(sessionId) {
  return shortcodeResolverOrNull()?.codeOf(sessionId) ?? null
}
...
/**
 * 名称真值 = 日志里最后一个 `session/title` 事件。
 *
 * 日志里确实没有 title 事件的会话（实测已知数条）⇒ 返回空串：**如实留空**。
 * 不回落成 cwd 项目名、也不拿 sessionId 前 16 位冒充名字 —— 虚构的名字经官方
 * rename 会固化成 user source title，把以后的真名字永久盖掉。
 *
 * 提到模块级并导出（`apply()` 之外）是为了可单测：这是"列表里显示什么名字"的
 * 唯一真值来源，回归代价高（见 test/badge-title-wiring.test.js 的阴性对照）。
 */
export function trueTitleOf(session) {
  const events = sessionEvents(session)
  for (let i = events.length - 1; i >= 0; i--) {
    if (events[i]?.type === 'session/title') return events[i].data?.title ?? ''
  }
  return ''
}

/** 组 `[短码] 名称`；名称为空 ⇒ 只留 `[短码]`（名字留空，不伪造）。 */
export function tagTitle(badge, title) {
  const clean = String(title ?? '').replace(/^\[[a-z0-9]{6,}\]\s*/, '')
  return clean ? `[${badge}] ${clean}` : `[${badge}]`
}

/** 该标题是否已带本短码（幂等判据：`[badge]` 或 `[badge] 名称`）。 */
export function hasBadgeTitle(badge, title) {
  const text = String(title ?? '')
  return text === `[${badge}]` || text.startsWith(`[${badge}] `)
}

/**
 * pinAllResident 的计数汇总（纯函数，便于单测）。
 * "无驻留会话不许伪造写入数"就钉在这里：所有计数**只来自 results**，
 * 空集合 ⇒ 全 0；带 error 的结果计入 failed，**不**计入 wrote。
 */
export function summarizePinResults(results, sessionCount) {
  const list = Array.isArray(results) ? results : []
  return {
    sessions: Number.isFinite(sessionCount) ? sessionCount : list.length,
    wrote: list.filter((r) => r.alreadyTagged === false).length,
    skipped: list.filter((r) => r.alreadyTagged === true).length,
    noTitle: list.filter((r) => r.titled === false).length,
...
export function apply(ctx) {
  ...
  /** 一次性日志（按 key 去重）:短码诊断每 30s 重跑,重复行会淹掉真信号。 */
  const loggedOnce = new Set()
  const logOnce = (key, line) => { if (loggedOnce.has(key)) return; loggedOnce.add(key); log(line) }
  ...
  /**
   * 短码相关 op 的**单一实现**（harness 与 HTTP 两条通道共用同一份函数）。
   *
   * 真回归（2026-09-19 重启实测）：`badgeMap` 只挂了 harness ⇒ App 走 HTTP ⇒ **404**。
   * 根因是"同一件事写两遍、挂了一条忘另一条"。这里把四条 op 收成一个对象，
   * 两条通道都从这个对象挂 ⇒ **结构上不可能只挂一条**（跨通道一致性由
   * test/op-channel-parity.test.js 再兜一道）。
   */
  /**
   * 短码四条 op 的**晚绑定 holder**。
   *
   * 为什么需要：这四条要调 `pinTitle` / `pinAllResident`，而它们在下面的
   * `disposeHarness` IIFE 闭包里（函数声明提升，只在那个闭包可见）。
   * 在这里**直接构造**会 ReferenceError（2026-09-20 重启实测：
   * `setBadgeTitle-failed: ReferenceError: pinTitle is not defined`）。
   * ⇒ 这里只声明**同一个对象引用**：HTTP handler 拿它当 `extraOps`，
   *   harness 侧也包一层从它取；真正的成员在 IIFE 里**无条件**写入。
   */
  const extraOpsHolder = {}
  ...
  const handler = createRetraceHttpHandler(ctx, {
    ...
    hooks,
    log,
    // HTTP 侧与 harness 侧**同一份实现**（真回归修复：badgeMap 曾只挂 harness ⇒ 404）。
    extraOps: extraOpsHolder,
  })
  ...
  const disposeHarness = (() => {
    // 短码四条 op：**在这里**构造（本闭包内 pinTitle/pinAllResident 可见），
    // 并**无条件**写入 holder —— 不放在任何 if 分支里，杜绝"没走到就没赋值"。
    Object.assign(extraOpsHolder, {
    // 会话短码（2026-08-31）：session id → 确定性 10 位短码，人机交互识别用。
    sessionBadge: async (args) => {
      const sessionId = String(args?.sessionId ?? '')
      return { ok: true, value: { sessionId, badge: await sessionBadge(sessionId) } }
    },
    // 会话短码写入标题（2026-09-01）：给会话 title 组装「[短码]原标题」。
    setBadgeTitle: async (args) => {
      const sessionId = String(args?.sessionId ?? '')
      if (!sessionId) return { ok: false, error: { code: 'bad-request', message: 'sessionId required' } }
      const session = ctx.sessions?.get?.(sessionId)
      if (!session) {
        logOnce(`setBadgeTitle:notfound:${sessionId}`, `retrace: setBadgeTitle ${sessionId} → not-found(会话不在 ctx.sessions)`)
        return { ok: false, error: { code: 'not-found', message: 'session not found' } }
      }
      try {
        const badge = await sessionBadge(sessionId)
        // 读当前 title（最后一个 session/title 事件；宿主 header **没有** title 字段）。
        // 无 title 事件 ⇒ 空串（如实留空，不用 sessionId 前 16 位冒充名字）。
        const currentTitle = trueTitleOf(session)
        const titled = currentTitle !== ''
        if (hasBadgeTitle(badge, currentTitle)) {
          return { ok: true, value: { title: currentTitle, badge, alreadyTagged: true, titled } }
        }
        const newTitle = tagTitle(badge, currentTitle)
        const wrote = pinTitle(session, newTitle)
        logOnce(`setBadgeTitle:${sessionId}:${newTitle}`, `retrace: setBadgeTitle ${sessionId} → ${newTitle}(rename=${wrote === true ? 'ok' : 'fail'})`)
        return { ok: true, value: { title: newTitle, badge, alreadyTagged: false, titled } }
      } catch (error) {
        return { ok: false, error: { code: 'internal', message: String(error) } }
      }
    },
    initBadgeTitles: async () => {
      // 同一闭包内 ⇒ 直接调（函数声明提升，晚绑定已不需要）
      const result = await pinAllResident()
      logOnce('initBadgeTitles:result', `retrace: initBadgeTitles → ${result?.ok === true ? `ok total=${result.value.total}` : `error ${result?.error?.message ?? ''}`}`)
      return result
    },
    // 短码全量映射：客户端同步渲染路径（横幅 labelOf）不能 await、也不能自算
    // （本地只有 FNV 兜底模块）⇒ 由宿主把已解析映射一次下发，保证跨载体同源。
    badgeMap: async () => {
      const resolver = shortcodeResolverOrNull()
      if (!resolver) return { ok: false, error: { code: 'unavailable', message: 'shortcode resolver unavailable' } }
      try {
        const badges = resolver.map()
        const tableError = resolver.stats().tableError ?? null
        logOnce('badgeMap:result', `retrace: badgeMap → ${Object.keys(badges).length} 条${tableError ? `;短码表有问题: ${tableError}` : ''}`)
        return { ok: true, value: { count: Object.keys(badges).length, badges, tableError } }
      } catch (error) {
        return { ok: false, error: { code: 'internal', message: String(error) } }
      }
    },    })
    ...
    async function ensureBadgeTitle(sessionId) {
      try {
        ...
        if (!badge) return
        // 名称一律取日志真值；无 title 事件 ⇒ 空串（留空，不伪造）
        const currentTitle = trueTitleOf(session)
        if (hasBadgeTitle(badge, currentTitle)) return
        pinTitle(session, tagTitle(badge, currentTitle))
      } catch { /* 标题写入失败不影响操作 */ }
    }
    /**
     * 名称真值 = 日志里最后一个 `session/title` 事件（实现见模块级 `trueTitleOf`，
     * 提到模块级是为了可单测）。
     */
    ...
    async function pinAllResident() {
      ...
      if (!sessionStore || (typeof sessionStore.list !== 'function' && typeof sessionStore.values !== 'function')) {
        logOnce('pinAllResident:nostore', 'retrace: pinAllResident → 会话存储不可枚举(unavailable)')
      ...
      try {
        ...
        logOnce('pinAllResident:enter', `retrace: pinAllResident → 驻留会话 ${sessions.length} 个`)
        for (const session of sessions) {
          ...
          try {
            const badge = await sessionBadge(sessionId)
            if (!badge) {
              // 此前完全静默:驻留数正常但"新写=0"时无法区分"没码"与"没会话"
              results.push({ sessionId, badge: null, error: 'no-badge(解析器无码)' })
              continue
            }
            // 名称取日志真值；无 title 事件 ⇒ 空串（留空，不伪造）
            const currentTitle = trueTitleOf(session)
            const titled = currentTitle !== ''
            if (hasBadgeTitle(badge, currentTitle)) {
              results.push({ sessionId, badge, alreadyTagged: true, titled })
              continue
            }
            pinTitle(session, tagTitle(badge, currentTitle))
            results.push({ sessionId, badge, alreadyTagged: false, titled })
          } catch (error) {
            results.push({ sessionId, badge: null, error: String(error) })
          }
        }
        const s = summarizePinResults(results, sessions.length)
        logOnce(`pinAllResident:sig:${s.sessions}/${s.wrote}/${s.skipped}/${s.noTitle}/${s.failed}`,
          `retrace: pinAllResident 汇总 → 驻留=${s.sessions} 新写=${s.wrote} 已带码=${s.skipped} 无标题(留空)=${s.noTitle} 失败=${s.failed}`)
        return { ok: true, value: { total: results.length, results, summary: s } }
      } catch (error) {
        return { ok: false, error: { code: 'internal', message: String(error) } }
      }
    }
    ...
    const bootPinRetry = createBootPinRetry({
      ...
      hasResidentSessions: () => {
        const sessionStore = ctx.sessions
        const has = Boolean(sessionStore && (
        ...
        // 打点:此前"无驻留会话"是完全静默的(20×30s 窗口内 0 条日志),
        // 现场无法区分"没跑"与"跑了但没会话"。去重后只留一行。
        if (!has) logOnce('bootPin:nosessions', 'retrace: bootPin → 驻留会话 0 个,本轮跳过(30s 后重试)')
        return has
      },
      log,
    })
     ...
     * 会话变"活"时立刻补一次 pin。
     *
     * 根因（本轮实测，源码一手）：`ctx.sessions.list()` 的官方语义是
     * "All **live** sessions, in creation order"（@deepseek-ai/dsh-session）。
     * 启动那一刻通常**一个活会话都没有** ⇒ 光靠 bootPin 的启动窗口永远覆盖不到
     * 用户后来才打开的会话（实测全天 0 行 bootPin 日志）。
     * 兄弟插件同款钩子：dsh-compaction `ctx.on("session/created", …, { global: true })`。
     */
    let sessionCreatedDispose = null
    try {
      sessionCreatedDispose = ctx.on('session/created', (session) => {
        const sessionId = session?.id ?? session?.sessionId
        if (!sessionId) return
        Promise.resolve()
          .then(() => ensureBadgeTitle(sessionId))
          .then(() => logOnce(`sessionCreated:${sessionId}`, `retrace: session/created ${sessionId} → 已补 pin 短码标题`))
          .catch((error) => logOnce(`sessionCreated:err:${sessionId}`, `retrace: session/created ${sessionId} pin 失败: ${String(error).slice(0, 120)}`))
      }, { global: true })
    } catch (error) {
      logOnce('sessionCreated:hookfail', `retrace: session/created 钩子注册失败: ${String(error).slice(0, 120)}`)
    }
    const disposers = [
      () => sessionCreatedDispose?.(),
      ...
      harness.handle('retrace.sessionBadge', (...a) => extraOpsHolder.sessionBadge(...a)),
      harness.handle('retrace.setBadgeTitle', (...a) => extraOpsHolder.setBadgeTitle(...a)),
      ...
      harness.handle('retrace.initBadgeTitles', (...a) => extraOpsHolder.initBadgeTitles(...a)),
      harness.handle('retrace.badgeMap', (...a) => extraOpsHolder.badgeMap(...a)),
