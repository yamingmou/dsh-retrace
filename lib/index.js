...
// issue-229:span 显式状态结果 → 业务层判定输入的统一下传(单一真相模块)
...
export function apply(ctx) {
  ...
  const disposeHarness = (() => {
    ...
    // 官方 foldSurface 重放)算出;issue-229 起返回显式状态 {status, span, facts}。
    ...
    const withFileSpan = (op, modeOf) => async (args) => {
      ...
      try {
        ...
        if (target !== undefined && target !== null && !args?.span) {
          ...
          // issue-200:spanProbe = span + 文件快照事实(同一份快照)。
          // issue-229(复核,显式状态):probe.status = SPAN_STATUS 五态——
