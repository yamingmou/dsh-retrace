...
export function apply(ctx) {
  ...
  const disposeHarness = (() => {
    ...
    const withFileSpan = (op, modeOf) => async (args) => {
      ...
      try {
        ...
        if (target !== undefined && target !== null && !args?.span) {
          ...
          // issue-200:spanProbe = span + 文件快照事实。文件读到但目标不在其 surface
          // (span null)时注入 args.spanFacts——host-core 区分「提交中(文件 flush 滞后,
          // message-pending)」与「真被遮蔽(target-shadowed)」,不再一律当遮蔽。
          const probe = await dshAdapter.spanProbeFromFile(args.sessionId, target, mode)
          log(`retrace: spanFromFile(op=${op} target=${String(target).slice(0, 24)} mode=${mode}) → ${probe?.span ? `${probe.span.shadowedSeqs.length} seqs` : `null(fallback fileMax=${probe?.facts?.fileMaxSeq ?? -1})`}`)
          if (probe?.span) {
            args = { ...args, span: probe.span }
            // M-1(独立审查 74e580d 后续):regenerate 的重发原文必须来自**该轮前置
          ...
          } else if (probe?.facts && typeof probe.facts.fileMaxSeq === 'number' && probe.facts.fileMaxSeq >= 0) {
            args = { ...args, spanFacts: probe.facts }
          }
        } else {
    ...
    const disposers = [
      ...
      // 关闭守卫(issue-146):查询全会话运行中状态(agent/queued/jobs/未闭合轮)。
  ...
  // 关闭守卫(issue-146 防误关)：退出/重载时对"运行中任务/未完成对话"会话强提示。
