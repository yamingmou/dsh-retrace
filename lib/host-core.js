 ...
 */
export async function appendEditorMarker(session, span, op, targetSeq, originalText, validate) {
  ...
  if (typeof validate === 'function') {
    await validate(session, { type: 'assistant/message', data, surfaceOp, sourceEventSeqs })
  }
...
export function createEditorApi(ctx, sessions, agents, log = () => {}, hooks = {}) {
  ...
  const editAndResend = op(async (args) => {
    ...
    if (!span) throw editorError('target-shadowed', 'This message is no longer part of the active conversation.')
    await appendEditorMarker(session, span, 'edit', seq, originalText, hooks.validateMarker)
    await flushSafely(session)
  ...
  const regenerate = op(async (args) => {
    ...
    if (!span) throw editorError('target-shadowed', 'This message is no longer part of the active conversation.')
    await appendEditorMarker(session, span, 'regenerate', userSeq, text, hooks.validateMarker)
    await flushSafely(session)
    const message = resendMessage(text.trim(), 'resend')
    agent.followup(message)
    return {
