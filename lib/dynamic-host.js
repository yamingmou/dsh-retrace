...
return {
  ...
  apply(ctx) {
    ...
    function createEditorApi(ctx, sessions, agents, log = () => {}, hooks = {}) {
       ...
       * `sourceEventSeqs`（seq 7000004：引用跨 turn 54 的 step 7/8/9），
      ...
      const regenerate = op(async (args) => {
        ...
        if (args?.span) {
          ...
          // (与旧循环语义逐字一致:孤儿回复不重发)。
          const startPos = roundStartIndex(nodes, idx - 1, (s) => isRoundBoundary(session.events[s]))
          if (isRoundBoundary(session.events[nodes[startPos]])) userSeq = nodes[startPos]
        }
     ...
     * ② 无打开 step 但有打开着的 turn（回合内 step 间隙）
     ...
     */
    // (契约运行时化):跨层边界校验——业务层传入的 span 与写出的
    ...
    function createDshMarkerWriter({ agents, validateMarker, readMaxStep, log = () => {} } = {}) {
      return {
        ...
        async writeMarker(session, span, meta) {
          // 契约边界:业务层 → 适配器。span 不合规(空/倒置/非连续段)
          ...
          if (openStep) {
            ...
            wrappedAfter = [
              ...
              // 缺失 = malformed → 官方 validation 拒绝 → 会话加载失败（
          ...
          // 契约边界:**可抛断言必须在任何 append 之前**。
          // 若先写 wrappedBefore/marker 再断言,违规时 wrappedAfter(step/end、turn/end)
          // 不会写入 → 日志里留下"半关闭 turn"(本插件历史事故形态),而调用方看到的是
          // 失败结果 + 内存面已被改。这里用**占位 seq 的信封预览**把形状校验提前:
          // seq 由 session.append 分配,是唯一无法提前得知的字段。
          const envelopePreview = { seq: 0, type: 'assistant/message', data, surfaceOp, sourceEventSeqs }
          assertMarkerShape(envelopePreview, 'dshAdapter.writeMarker.marker(preview)')
