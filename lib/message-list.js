 ...
 */
/**
 * 消息形态的轮边界谓词(与 lib/span-semantics.js 的 isRoundBoundaryEvent 同义):
 * 只有**真实用户输入**才是轮边界。消息带 `source` 时按 `source.kind === 'user'` 判
 * (注入上下文/steering 的 kind 不是 'user',遮蔽载体的 kind 是 'model');
 * 消息层没有 source 维度时回落 `role === 'user'`。
 *
 * 单独导出而不是内联:这条谓词是"预览 = 写入"的一端,也是载体改造的红线
 * (载体写 `kind:'user'` 会让每一轮边界整体偏移)——导出后验收脚本可直接断言
 * 投影结果上它返回 false,不必靠间接推断。
 * @param {{role?:string, source?:{kind?:string}}} message - 投影后的消息(适配器产物)
 */
export function isRoundBoundaryMessage(message) {
  if (!message) return false
  if (message.source && typeof message.source.kind === 'string') {
...
export function shadowSpanOf(messages, shadows, targetSeq, { mode = SPAN_MODE.TAIL } = {}) {
  ...
  return spanForSeq(nodes, targetSeq, {
    mode,
    isBoundary: (seq) => isRoundBoundaryMessage(bySeq.get(seq)),
  })
}
