 ...
 */
// issue-229 第 2 项:遮蔽范围规则(轮首回退/尾部切片)= lib/span-semantics.js 单一真相。
 ...
 *
 * issue-229 第 2 项（复核）：**切片规则不在本文件实现** —— 轮首回退 / 尾部切片
 ...
 * 轮边界谓词 = role === 'user'（消息形态没有 source 维度；事件形态用
 * isRoundBoundaryEvent，排除注入上下文——两形态的差别显式写在各自调用点）。
 *
...
export function shadowSpanOf(messages, shadows, targetSeq, { mode = SPAN_MODE.TAIL } = {}) {
  ...
  const roleOf = new Map(active.map((m) => [m.seq, m.role]))
  return spanForSeq(nodes, targetSeq, {
    mode,
    isBoundary: (seq) => roleOf.get(seq) === 'user',
  })
}
