 ...
 *   - replace ranges are EVENT SEQS (not surface indices); the shadowed seqs
 *     are the surface nodes the splice removed — exactly
 *     `foldSurface(...).replacements[].shadowedSeqs`. Host 0.1.5 spells the
 *     range `{startSeq,endSeq}`, the 0.1.x dev host `{start,end}` — read both
 *     through lib/marker-carrier.js#spanRangeOf.
 ...
 *     `retrace/versions` by boundarySeq). The human-readable digest of WHAT a
 *     boundary replaced (`excerpt` / `artifacts` / optional `summary`) is the
 *     plugin's own artifact, served by its own route — never the projection
 *     wire or the durable checkpoint.
 *
...
import {
  ...
  isSurfaceEvent,
  replacedSeqsOfBoundary,
} from './version-index.js'
import { spanRangeOf } from './marker-carrier.js'
...
export function applyForkmap(state, event) {
  ...
  if (isSurfaceEvent(event)) {
    ...
    if (op === 'append') {
      ...
      const startIdx = range === null ? -1 : next.nodes.findIndex((node) => node.seq === range.start)
      const endIdx = range === null ? -1 : next.nodes.findIndex((node) => node.seq === range.end)
  ...
  if (isReplacementSurfaceEvent(event)) {
    const boundary = {
      ...
      replacedSeqs: replacedSeqsOfBoundary(event, shadowed),
    }
