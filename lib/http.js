...
function sendError(res, error) {
  sendJson(res, 200, {
    ok: false,
    error: {
      // issue-200:editorError 附带的排查信息(messageId/seq)透传到 wire(同 op() 信封)
      ...
      if ((op === 'recall' || op === 'editAndResend' || op === 'regenerate') && !args?.span) {
        try {
          ...
          if (target !== undefined && target !== null && !args?.span) {
            ...
            // L-1(独立审查 74e580d 后续):与 index.js harness 入口对齐——**单次**
            ...
            if (probe?.span) {
              ...
              // M-1(独立审查 74e580d 后续):regenerate 的重发原文 = 该轮前置
              ...
              probe?.facts && typeof probe.facts.fileMaxSeq === 'number' && probe.facts.fileMaxSeq >= 0
            ) {
              // issue-200:span null → 注入文件快照事实(与 span 同一份快照),host-core
              // 据此区分「提交中(文件 flush 滞后,message-pending)」与「真被遮蔽
              // (target-shadowed)」。
