...
// issue-229:span 显式状态 → 业务层判定输入(与 index.js harness 入口同一处理)
        ...
        try {
          ...
          if (target !== undefined && target !== null && !args?.span) {
            ...
            if (probe?.span) {
              ...
              // issue-229:按**显式状态**下传(不再靠"span 为 null + 事实"猜)——
              ...
              //
              // ⚠️ 投影安全(独立审查 issue-229 阻断项):这里**不能**用「行级删除标记」挂在
              // `else if (op !== 'fold') {` 那一行上——整行(含收尾 `}` 与开头 `{`)被删掉后,
              // 分支体被上层 if 吸收成死代码(公开线 HTTP 入口状态判定全失效,语法却仍合法)。
