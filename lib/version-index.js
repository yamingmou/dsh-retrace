 ...
 *
 * HOST-SIDE surface replacements are FILTERED OUT here (kind `replace`: a
 * replacement whose event is neither one of our carriers nor an official
 * compaction checkpoint — measured on the real session: 27/61 boundaries, all
 * `tool/result` + a plain UUID id, e.g. the host re-rendering a tool result).
 * They are not the user's changes; listing them made the read point look like
 * "替换/替换/替换" with no content (real-machine finding 2026-09-15). The count
 * travels as `hostReplacementCount` so the view can say how many were hidden
 * instead of silently dropping them.
 *
 * The RAW fold state keeps every boundary: `artifactCountsOf` /
 * `boundarySeqsOf` (write side) read it, and the durable checkpoint contract is
 * unchanged (no `stateVersion` bump).
 */
export function viewVersionIndex(state) {
  const all = Array.isArray(state?.versions) ? state.versions : []
  const mine = all.filter((record) => record?.kind !== 'replace')
  return {
    versions: mine.map((record) => ({
    ...
    hostReplacementCount: all.length - mine.length,
  }
}
