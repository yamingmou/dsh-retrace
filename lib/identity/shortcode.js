...
/**
 * 读短码表 → `{ entries, error, path, missing }`。**坏表不静默**:
 * 文件读不了 / JSON 解析失败 / `codes` 形状不对 / 有条目不是非空字符串 ——
 * 都从 `error` 出来（调用方要把它报出来）。旧实现 `catch { return [] }` 会把
 * "表坏了"伪装成"表是空的",于是短码悄悄退化成按行派生码,没人知道为什么。
 */
export function loadTableEntriesSafe(path = resolveBadgeTablePath()) {
  if (!existsSync(path)) return { entries: [], error: null, path, missing: true }
  let raw
  try { raw = readFileSync(path, 'utf8') } catch (error) {
    return { entries: [], error: `unreadable: ${error?.message ?? error}`, path, missing: false }
  }
  let parsed
  try { parsed = JSON.parse(raw) } catch (error) {
    return { entries: [], error: `malformed-json: ${error?.message ?? error}`, path, missing: false }
  }
  const codes = parsed?.codes
  if (codes === null || typeof codes !== 'object' || Array.isArray(codes)) {
    return { entries: [], error: 'bad-shape: `.codes` 应为非数组对象', path, missing: false }
  }
  const keys = Object.keys(codes)
  const entries = Object.entries(codes)
    .filter(([, v]) => typeof v === 'string' && v !== '')
    .map(([k, v]) => ({ code: v, sessionId: k, source: 'badge-table' }))
  if (entries.length !== keys.length) {
    return { entries, error: `bad-entries: ${keys.length - entries.length}/${keys.length} 条短码不是非空字符串`, path, missing: false }
  }
  return { entries, error: null, path, missing: false }
}

/** 读短码表 → 已登记项（兼容旧签名）。**键形态原样保留**，归一只用于连接。 */
export function loadTableEntries(path = resolveBadgeTablePath()) {
  return loadTableEntriesSafe(path).entries
}
...
export function loadUniverse(opts = {}) {
  ...
  const table = loadTableEntriesSafe(tablePath)
  return { sessions: rows, pinned: table.entries, entries: table.entries, tablePath, tableError: table.error, tableMissing: table.missing, idmapPath, parentEdges, root, errors }
}
...
export function createShortcodeResolver(opts = {}) {
  ...
  return {
    ...
    /**
     * 全量映射（批量下发/诊断用）：verbatim session id → 规范短码。
     * 客户端渲染路径不能 await，也不能自己算（本地只有 FNV 兜底模块）⇒
     * 由宿主把已解析的映射一次交给它，让横幅与读档点视图跨载体同源。
     */
    map() {
      state ??= build()
      const out = {}
      for (const row of state.uni.sessions) {
        const code = state.byUuid.get(row.uuid)?.code
        if (typeof code === 'string' && code !== '') out[row.id] = code
      }
      return out
    },
    /** 丢掉缓存（会话增删后调用）。 */
    ...
    stats() {
      ...
      return {
        ...
        tableError: state.uni.tableError ?? null,
        kinds,
