...
import { carrierContentText, isCarrierMarkerEvent } from '../marker-carrier.js'
...
/** 找 DSH 会话文件路径(实现见 lib/platform/session-paths.js:候选根逐工作区探测
 * 两种文件名)。保留同名导出:此前本文件自带一份,外部引用面不变。 */
...
/** 从指定文件路径读全量事件(测试可注入路径;生产走 readEvents 按基座候选找)。
...
export async function deriveBadgeTable(opts = {}) {
  ...
  for (const { workspace, file } of listSessionFiles(root)) {
  ...
  // 工作区内按 createdAt 排序 → 序号(与短码表生成器一致)
  const byWs = {}
