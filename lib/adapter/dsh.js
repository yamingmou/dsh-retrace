...
export async function readEventsFromFile(filePath) {
  try {
    ...
    // "历史只读"误报——正是 issue-229 要消灭的误导(独立审查 issue-229 中-4)。
...
export function computeSpan(events, target, mode = 'round') {
  ...
  if (seq === -1) {
    ...
    //    (独立审查 issue-229 中-6:原实现一律 not-persisted 属无证据推断,会把永久不存在
    //     的目标报成"可重试"。)
  ...
  try {
    ...
    // 独立审查 issue-230 :同一状态由两种原因共用,靠 facts.cause/message **判因**
  ...
  if (!Array.isArray(nodes) || nodes.length === 0) {
    ...
    // + nodes=0 区分(独立审查 issue-230 :两者都只报"内部错误"时无法判因)。
