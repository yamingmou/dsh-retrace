 ...
 *                                       turn 信封推进 agent-loop 计数器（appendEditorMarker 情形③）。
 * @param {(line: string) => void} [deps.log]
 */
export function createRollbackExecutor({ ctx, sessions, agents, seam, validateMarker, log = () => {} }) {
    ...
    return locked(sessionId, async () => {
      ...
      if (scope === 'context' || scope === 'both') {
        ...
        if (diff.length > 0) {
          ...
          const markerEvent = await appendEditorMarker(session, span, 'restore', record.boundarySeq, '', validateMarker, agents?.get?.(sessionId))
