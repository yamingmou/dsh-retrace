 ...
 */
export function createRollbackExecutor({ ctx, sessions, seam, validateMarker, log = () => {} }) {
    ...
    return locked(sessionId, async () => {
      ...
      if (scope === 'context' || scope === 'both') {
        ...
        if (diff.length > 0) {
          ...
          const markerEvent = await appendEditorMarker(session, span, 'restore', record.boundarySeq, '', validateMarker)
