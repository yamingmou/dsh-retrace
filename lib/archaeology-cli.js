...
// 基座换代后考古会找不到会话。
import { sessionFilePath } from './platform/session-paths.js';
...
/** 解析会话参数：完整文件路径，或 sessionId（在 sessions 基座候选下查找）。 */
export function resolveSessionFile(ref) {
  ...
  // 按 id 在所有工作区查找（基座/文件名探测见 lib/platform/session-paths.js）
  const hit = sessionFilePath(ref);
  if (hit) return hit;
  throw new Error(`session "${ref}" 未在 sessions 基座（$DSH_HOME/~/dsh-v3/~/.dsh）下找到`);
}
