 ...
 * SEPARATE from the state schema: `digests` / `pendingSeq` are fold internals
 * and must not travel in push frames (the view is ~92 KB smaller without them).
 */
export const forkmapViewSchema = z.object({
  nodes: z.array(forkmapNodeSchema),
  boundaries: z.array(forkmapBoundarySchema),
})
