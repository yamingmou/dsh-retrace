 ...
 *
 * 关闭守卫(issue-146,2026-09-09)——防误关:会话有**运行中任务**时关闭会中断丢进度。
 *
 * 背景:用户刚失误关闭有运行中任务的对话损失进度。issue-146 小任务。
 *
 * Seam 调研结论(工程-插件需求/关闭守卫-seam调研与方案-20260909.md):
 ...
 * - 故交付替代路径(issue-146 允许):①运行中**检测全集**(含 jobs/queued,比只查未闭合
...
export function runningState(sessionId, { agent, session, jobs = [] } = {}) {
  ...
  // 2) 排队/待办 work(inbox 非空——status=idle 也可能有 queued)
  const inbox = agent?.inbox
  const queued = inbox && typeof inbox === 'object'
    ? (Array.isArray(inbox.queued) ? inbox.queued.length
      : (Array.isArray(inbox.pending) ? inbox.pending.length : 0))
    : 0
  if (queued > 0) reasons.push(`queued-${queued}`)
  // 3) 未闭合轮(崩溃/强杀现场;interrupted/aborted 是官方正常闭合不算——复用 interrupt-guard)
  const unclosed = unclosedTurnsOf(session)
  if (unclosed.length > 0) reasons.push(`unclosed-turn-${unclosed.map((u) => u.turn).join(',')}`)
  // 4) 后台任务(jobs 关联本会话)
  if (Array.isArray(jobs)) {
    const mine = jobs.filter((j) => j && (j.sessionId === sessionId || j.session === sessionId))
    if (mine.length > 0) reasons.push(`jobs-${mine.length}`)
  }
  return { sessionId, running: reasons.length > 0, reasons }
}

/** interrupt-guard 的 unclosedTurns 同款检测(避免循环依赖,内联轻量版)。 */
function unclosedTurnsOf(session) {
  ...
  if (openTurn !== null) found.push({ turn: openTurn, state: 'open' })
  return found
}
...
export function runningSessions(ctx) {
  ...
  const jobs = (() => {
    try {
      ...
      const list = typeof service?.list === 'function' ? service.list() : null
