 ...
 */
// P0-6(代码低 2):unclosedTurns 直接复用 interrupt-guard(该文件零 import,无环),
 ...
 *  P0-2(四角度审核 E-1/E-2 = 代码 H-1):对齐官方形状——agent.inbox 公开面是
...
/**
 * 读取 jobs 快照。P0-2:官方 list(caller) 语义 = 只返回无主或属于该 caller 的 job;
...
export function runningSessions(ctx) {
  ...
  for (const id of ids) {
    ...
    // P0-2:按会话取 jobs(官方 list(caller={id}) 只返该会话 job;读不到降级空)
