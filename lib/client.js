 ...
 * 行数 ≤ 本值的会话永不触发 40% 降级——否则 5-10 行的小会话撤回/编辑
 * 2-4 条消息就 >40%，被误判为「大范围操作」降级为不隐藏（用户实测：
 * 「已撤回 4 条消息 + 此操作涉及大范围对话…」= 小会话误伤）。
 * 与 host 侧 ROLLBACK_MIN_SURFACE=6（surface 节点）同源：短会话豁免只在
 * 大会话按比例判定。10 行 ≈ 5 轮，正常撤回/编辑远小于此。
 */
const SHADOW_MIN_ROWS_FOR_RATIO = 10
