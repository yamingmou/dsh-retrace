...
const zh = {
  ...
  'options.hideShadowedDesc': '开：撤回/编辑/重新生成按标记隐藏被替换的消息。关（默认）：所有消息保持可见，标记仅显示提示与对照（查看完整历史用）。一次操作要隐藏超过 40% 的对话时自动降级为不隐藏。',
...
const en = {
  ...
  'options.hideShadowedDesc': 'On: recall/edit/regenerate hide the replaced messages per their markers. Off (default): every message stays visible; markers only show the notice and reference (use to review full history). A single op that would hide more than 40% of the conversation degrades to notice-only automatically.',
...
// ---------------------------------------------------------------------------
/**
 * v2 defaults (2026-08-26 incident fix): v1's defaults (`editFromScratch: true`
 * + `hideShadowed: true`) made a single edit hide the WHOLE conversation before
 * the edit point, so clicking "load earlier" appeared to load nothing. v2 makes
 ...
 *  - `hideShadowed: false` — recalled/edited rows stay visible; markers render
 *    as notice rows (full history always reviewable). Users who want the
 *    "rewind look" can re-enable hiding; the safety guard in `useHiddenKeys`
 *    caps how much a single marker may hide.
 */
const CONFIG_VERSION = 2
const CONFIG_DEFAULTS = { version: CONFIG_VERSION, showOriginalInput: true, editFromScratch: false, hideShadowed: false, versioning: true, git: true, retentionLimit: 50, prewrite: true }
...
/**
 * v1 → v2 migration: v1's defaults made one edit hide the whole conversation.
 * v2 resets exactly that pair (`editFromScratch` / `hideShadowed`) to the safe
 * defaults; every other customization is preserved.
 */
  ...
  try {
    ...
    if (raw !== null) {
      ...
      if (storedVersion < CONFIG_VERSION) {
        // Persist the migration so the next load starts from the v2 shape.
...
/**
 * If one marker would hide more than this share of the conversation's rows,
 * the hiding is refused (the marker still renders as a notice). A single
 * recall/edit must never blank out most of the history — the rows stay in the
 * durable log and remain visible.
 */
...
function useHiddenKeys(useSession, shadowedSeqs) {
  return useSession((snapshot) => {
    ...
    // Safety guard (2026-08-26 incident): a marker covering a huge share of the
    // conversation (e.g. an edit with "fresh conversation" semantics) degrades
    // to notice-only — history must never silently vanish from the view.
