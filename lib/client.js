...
export const zh = {
  ...
  'timeline.intro': '每次撤回、编辑、重新生成前自动存一档，可回到任一档。',
  'view.retrace': '读档点',
  'timeline.refresh': '刷新',
  ...
  'tree.changes': '这条路里还有 {count} 次改动',
  'quiet.note': '（无输出、无产物变化）',
  'quiet.merged': '连续 {count} 次{kind}',
  'quiet.change': '改动',
  'quiet.expand': '▸ 展开',
  'path.title': '现在的路',
  'path.header': '现在的路（{messages} 条消息 / {rounds} 轮）',
  'path.round': '第 {n} 轮',
  'path.empty': '（当前没有对话轮次）',
...
export const en = {
  ...
  'timeline.intro': 'A checkpoint is saved automatically before every recall, edit or regenerate — jump back to any of them.',
  ...
  'tree.changes': '{count} more changes on this path',
  'quiet.note': ' (no output, no artifact changes)',
  'quiet.merged': '{count} quiet {kind} in a row',
  'quiet.change': 'changes',
  'quiet.expand': '▸ Show',
  'path.title': 'Current path',
  'path.header': 'Current path ({messages} messages / {rounds} rounds)',
  'path.round': 'Round {n}',
...
.dsh-rt-version{position:absolute;left:0;right:0;height:112px;box-sizing:border-box;display:flex;align-items:flex-start;gap:8px;border:1px solid transparent;border-radius:10px;padding:6px 8px;overflow:hidden}
...
.dsh-rt-what-impact{color:var(--dsw-alias-state-warning-primary);font-size:11px;line-height:16px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
...
.dsh-rt-tree-toggle{position:absolute;left:0;right:0;height:112px;box-sizing:border-box;display:flex;align-items:center;border:1px solid transparent;border-radius:10px;overflow:hidden}
...
/* ---- plain-text rows: fallback (what missing) and merged quiet runs ---- */
.dsh-rt-fallback,.dsh-rt-quiet-run{position:absolute;left:0;right:0;height:112px;box-sizing:border-box;display:flex;align-items:center;gap:8px;border:1px solid transparent;border-radius:10px;padding:6px 8px;overflow:hidden}
...
.dsh-rt-quiet-run .dsh-rt-plain-text{color:var(--dsw-alias-label-tertiary)}
...
function artifactsLabel(artifacts, t) {
  if (artifacts === undefined || artifacts === null) return null
 ...
 * server forest. Rows are `{kind: 'row'|'quiet'|'collapsed'|'depth'|'quietRun', …}`.
 *
 * - `tree === null` ⇒ every row is a level-0 current-path row (flat).
 * - A node with children is expanded by default when it is a ROOT (parent
 *   null); deeper nodes collapse to `还有 N 层 ▸`. Level > 3 folds to a hint.
 * - Nested rows never render 「延续」 (those paths have no continuation).
 * - ≥2 consecutive quiet rows merge into one expandable plain-text row.
 * Nesting is NEVER inferred by the client (no numeric ranges) — only the
 * server's `parent`/`children` links are followed.
 */
function buildDisplayRows({ versions, digests, tree, expanded }) {
  const list = Array.isArray(versions) ? versions : []
  const digestOf = (seq) => (digests instanceof Map ? digests.get(seq) : undefined)
  // Quiet = no `what` digest (no output / no run / no artifact change). The
  // host writes no artifact content for it, so it can only be plain text.
  const isQuiet = (seq) => {
    const digest = digestOf(seq)
    return digest === undefined || digest === null || digest.what === undefined || digest.what === null
  }
  ...
  // Default FLAT: a node expands only when the user toggled it.
  const isExpanded = (seq) => expanded instanceof Map && expanded.get(seq) === true
  ...
  const emitNode = (seq, level) => {
    ...
    const digest = digestOf(seq)
    const quiet = isQuiet(seq)
    rows.push({
      kind: quiet ? 'quiet' : 'row',
      record,
      digest,
      level,
    ...
    if (level >= TREE_MAX_LEVEL) {
      rows.push({ kind: 'depth', level, remaining: depthBelow(seq), count: descendants(seq) })
      return
    }
    if (!isExpanded(seq)) {
      rows.push({ kind: 'collapsed', seq, level: level + 1, remaining: depthBelow(seq), count: descendants(seq) })
      return
    }
    emitGroup(kids, level + 1)
  }
  // Consecutive quiet siblings (>=2) merge into ONE plain-text row; a lone quiet
  // row stays its own plain-text line (never blank).
  const emitGroup = (seqs, level) => {
    ...
    while (i < seqs.length) {
      ...
      if (isQuiet(seq)) {
        let j = i
        while (j + 1 < seqs.length && isQuiet(seqs[j + 1])) j += 1
        const run = seqs.slice(i, j + 1)
        if (run.length >= 2) {
          const runKey = `quiet:${run[0]}`
          if (isExpanded(runKey)) {
            for (const q of run) emitNode(q, level)
          } else {
            rows.push({ kind: 'quietRun', records: run.map((q) => bySeq.get(q)).filter(Boolean), quietSeqs: run, runKey, level, current: false })
          }
          i = j + 1
          continue
        }
...
function RetraceView({ sessionId, useChat, useProjection, t, actions, store }) {
  ...
  // Explicit expand toggles: seq → bool (plus `quiet:<seq>` runs).
  ...
  // Taller than 0.4.x: a row now carries the verbatim old excerpt + optional
  // summary + impact + artifact line, not just kind/time/counts.
  const ROW_H = 116
  const list = versions ?? []
  // Outline display list (flat when `tree` is absent; nested per the server forest).
  const rows = buildDisplayRows({ versions: list, digests, tree, expanded })
  // 2026-08-30 渲染卡死修复：带起始索引切片，渲染用索引算 top——
  // 原 `list.indexOf(record)` 是 O(N²)（每可见行线性查找），大会话渲染风暴。
  const visibleStart = Math.max(0, Math.floor(scrollTop / ROW_H) - 2)
  const visibleEnd = Math.min(rows.length, Math.ceil(scrollTop / ROW_H) + Math.ceil(640 / ROW_H) + 2)
  ...
  return createElement('div', { className: 'dsh-rt-view' }, [
    ...
    createElement('div', { key: 'intro', className: 'dsh-rt-view-intro' }, t('timeline.intro')),
    ...
    rows.length > 0 && createElement('div', {
      ...
      createElement('div', { key: 'spacer', style: { height: `${rows.length * ROW_H}px`, position: 'relative' } }, [
        visible.map((row, i) => createElement(CheckpointRow, {
          key: row.record?.versionId ?? row.runKey ?? `${row.kind}:${row.seq ?? i}`,
          row,
          t,
          top: (visibleStart + i) * ROW_H,
          onToggle: toggle,
          onPreview: () => requestPreview(row.record, { contextOnly: row.kind === 'quiet' }),
          onJump: () => jump(row.record.boundarySeq),
        })),
      ]),
    ]),

    // 最下方「现在的路」：默认折叠，展开后按轮次列对话文字（仅活路可跳）。
    createElement(CurrentPathBlock, { key: 'path', t, rounds, open: pathOpen, messages: chatNodesRef.current instanceof Map ? chatNodesRef.current.size : 0, onToggle: () => setPathOpen((open) => !open), onJump: (seq) => jump(seq) }),
 ...
 *
 * Quiet rows are PLAIN TEXT ONLY: no quote block, no summary block, no artifact
 * line, and the rollback action is context-only (the artifacts did not change).
 */
function CheckpointRow({ row, t, top, onToggle, onPreview, onJump }) {
  const level = Number.isInteger(row.level) ? row.level : 0
  const levelClass = level >= 1 && level <= TREE_MAX_LEVEL ? ` dsh-rt-level-${level}` : ''
  const currentClass = row.current ? ' dsh-rt-current' : ''
  const style = { top: `${top}px`, paddingLeft: `${Math.min(level, TREE_MAX_LEVEL) * 14}px` }
  ...
  if (row.kind === 'depth') {
    return createElement('div', { className: `dsh-rt-tree-toggle dsh-rt-tree-depth${levelClass}${currentClass}`, style }, [
      createElement('span', { className: 'dsh-rt-tree-hint' }, t('tree.moreLevels', { count: Math.max(1, row.remaining) })),
    ])
  }
  if (row.kind === 'quietRun') {
    const uniform = row.records.length > 0 && row.records.every((record) => record.kind === row.records[0].kind)
    const kind = uniform ? kindLabel(row.records[0].kind, t) : t('quiet.change')
    return createElement('div', { className: `dsh-rt-quiet-run${currentClass}`, style }, [
      createElement('span', { className: 'dsh-rt-plain-text' },
        `${t('quiet.merged', { count: row.records.length, kind })}${t('quiet.note')}\uff1a${row.quietSeqs.join('\u2192')}`),
      createElement('button', { type: 'button', className: 'dsh-rt-chip dsh-rt-quiet-btn', onClick: () => onToggle(row.runKey) }, t('quiet.expand')),
    ])
  }
  if (row.kind === 'quiet') {
    return createElement('div', { className: `dsh-rt-version dsh-rt-plain${levelClass}${currentClass}`, style }, [
      ...
      createElement('div', { key: 'body', className: 'dsh-rt-version-body' }, [
        createElement('div', { key: 'line1', className: 'dsh-rt-version-line' }, [
          ...
          createElement('span', { key: 'note', className: 'dsh-rt-plain-note' }, t('quiet.note')),
        ]),
      ]),
 ...
 */
function CurrentPathBlock({ t, rounds, open, onToggle, onJump, messages }) {
 ...
 */
function VersionRow({ record, top, t, level = 0, current = false, nested = false, what, summary, summaryCalled, discardedCount, onPreview, onJump }) {
  const summaryText = summaryCalled === true && typeof summary === 'string' && summary.trim() !== '' ? summary : null
  const digestBody = what === null || what === undefined ? [] : whatBody(what, summaryText, t, { suppressContinue: nested })
  const hasDigest = digestBody.some((element) => element !== null)
  const replacedTotal = (Array.isArray(what?.replaced) ? what.replaced.length : 0) + (Number.isInteger(what?.replacedMore) ? what.replacedMore : 0)
  ...
  return createElement('div', { className: `dsh-rt-version${levelClass}${currentClass}`, style: { top: `${top}px`, paddingLeft: `${Math.min(level, TREE_MAX_LEVEL) * 14}px` } }, [
    ...
    createElement('div', { key: 'body', className: 'dsh-rt-version-body' }, [
      ...
      createElement('div', { key: 'line1', className: 'dsh-rt-version-line' }, [
        ...
        // The digest carries the human "what changed"; only fall back to the raw
        // conversation size when there is no digest for this boundary.
        hasDigest ? null : createElement('span', { key: 'msgs', className: 'dsh-rt-version-msgs' }, t('timeline.messages', { count: record.messageCount })),
      ]),
      // 被丢弃（逐字原文）→ 可选摘要 → 延续（新原文）
      ...digestBody,
      replacedTotal > 0
        ? createElement('div', { key: 'impact', className: 'dsh-rt-what-impact' }, t('what.discarded', { count: impactCount }))
        : (hasDigest ? null : createElement('div', { key: 'text', className: 'dsh-rt-version-text' },
            record.markerText
              ? (record.markerText.length > 120 ? `${record.markerText.slice(0, 120)}…` : record.markerText)
              : whyLabel(record.kind, t))),
      // 产物变更：字段缺失 ⇒ 整行不显示（不出现 0/0/0）
      artifacts !== null
        ? createElement('div', { key: 'files', className: 'dsh-rt-version-files' }, artifacts)
        : null,
    ]),
    // 行尾动作：回到这一档 / 跳转
    createElement('span', { key: 'actions', className: 'dsh-rt-version-actions' }, [
...
function PreviewBox({ preview, scope, setScope, busy, t, onConfirm, onCancel }) {
  ...
  return createElement('div', { className: 'dsh-rt-modal' }, [
    ...
    data && createElement('div', { key: 'body', className: 'dsh-rt-modal-body' }, [
      ...
      !contextOnly && createElement('div', { key: 'art', className: 'dsh-rt-modal-line' }, t('timeline.artifactsList', { count: data.artifacts?.rows?.length ?? 0 })),
      !contextOnly && createElement('ul', { key: 'files', className: 'dsh-rt-modal-files' }, (data.artifacts?.rows ?? []).slice(0, 12).map((row) =>
 ...
 *   被丢弃（每条逐字旧原文，弱化灰 + 标记已丢弃）→ 可选【摘要】（独立元素）→
 *   延续（新原文，强调色）。
 * Verbatim quotes and the AI summary are ALWAYS separate elements.
 */
function whatBody(what, summaryText, t, { suppressContinue = false } = {}) {
  ...
  return [
    ...replaced.map((entry) => createElement('div', { key: `old-${entry.seq}`, className: 'dsh-rt-what-quote dsh-rt-what-old' }, [
      createElement('span', { key: 'void', className: 'dsh-rt-what-void' }, t('what.oldVoid')),
      createElement('span', { key: 'role', className: 'dsh-rt-what-role' }, roleLabel(entry.role, t)),
      createElement('span', { key: 'seq', className: 'dsh-rt-what-seq' }, `#${entry.seq}`),
      createElement('span', { key: 'text', className: 'dsh-rt-what-text' }, `「${entry.excerpt}」`),
    ])),
    replacedTotal > replaced.length
      ? createElement('div', { key: 'old-more', className: 'dsh-rt-what-more' }, t('what.more', { count: replacedTotal - replaced.length }))
      : null,
    summaryText !== null && summaryText !== undefined
      ? createElement('div', { key: 'summary', className: 'dsh-rt-what-summary' }, [
          createElement('span', { key: 'tag', className: 'dsh-rt-what-summary-tag' }, t('what.summaryTag')),
          createElement('span', { key: 'text', className: 'dsh-rt-what-summary-text' }, summaryText),
        ])
      : null,
    !suppressContinue && newExcerpt !== ''
      ? createElement('div', { key: 'new', className: 'dsh-rt-what-quote dsh-rt-what-new' }, [
          createElement('span', { key: 'tag', className: 'dsh-rt-what-new-tag' }, t('what.currentLabel')),
          createElement('span', { key: 'text', className: 'dsh-rt-what-text' }, `「${newExcerpt}」`),
        ])
      : null,
  ]
}
