...
function UserActionsRow({ node, sessionId, useSession, inputActions, t }) {
  ...
  // marker actually hides its message. Judge by the REAL message seq — the
  // user-actions node anchors at that seq (so it would match anyway), and it
  // keeps the reference node (seq-0.5) consistent with the same rule.
  const hidden = useSeqHidden(useSession, seq)
