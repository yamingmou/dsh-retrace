...
//    页面里分不出宿主属于哪一类,而第一类上否决是静默的(会卡住退出),所以桌面端
//    一律不武装。承载面由宿主随 runningState 载荷下发(quitVeto === true 才武装;
//    false/未知都不武装,未知取安全侧)。客户端不再自己猜宿主;
...
function installCloseGuard(t) {
  ...
  function refresh() {
    ...
    queryRunningState().then((payload) => {
      ...
      // quitVetoOf)。**原样缓存宿主的值**:缺字段 = 旧宿主 = 未知 ⇒ 不武装。
  ...
  function onBeforeUnload(event) {
    ...
    // 壳静默吞掉、退出卡住)。仅当宿主明确回报 quitVeto === true(普通浏览器页面)
    // 才武装;false 与未知一律不武装(未知取安全侧)。
    if (!quitVetoOf(store.get())) return
...
export function apply(ctx) {
  ...
  // 确认,运行中状态经 /runningState 轮询缓存)+ 运行中横幅。原生门只在宿主
  // 回报本页承载 quit-veto 时武装(见 installCloseGuard 段头注释);桌面端的
  // 可见提示 = 运行中横幅 + dispose 提示(与退出路径无关)。
