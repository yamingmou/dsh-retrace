...
/** Same as useRowHidden, but resolves the row key from a surface seq. */
...
function AssistantActions({ messageId, sessionId, useSession, t }) {
  ...
  if (hidden || seq === undefined) return null
...
function UserActionsRow({ node, sessionId, useSession, inputActions, t }) {
  ...
  // Mirror the CSS reality: the row stays operable unless a NON-degraded
  // marker actually hides it (a degraded marker hides nothing, so its
  // shadowed rows remain visible and editable — 0.4.3 kept them visible but
  // took the actions away).
  const hidden = useRowHidden(useSession, node.key)
