...
export const zh = {
  ...
  'options.closeGuardDesc': '开（默认）：页面关闭前确认退出——有运行中任务或未完成对话时强拦并列出明细（防误关丢进度），无任务时轻确认一次。只在宿主确认会显示原生确认框的页面上武装（普通浏览器标签页；关掉本项同时停用运行中横幅）。桌面端（DSH Desktop）一律不武装：退出入口随版本/平台而变——外部报告的 0.9.0 一类入口会触发页面 beforeunload，而桌面壳未处理 will-prevent-unload ⇒ 否决被静默吞掉、退出卡住；2.0.9 的托盘项走 requestQuit→window.destroy→app.exit，根本不经过它。判据是两道：宿主说会弹原生框（quitVeto），且本页自己看不出桌面痕迹（UA 无 Electron、URL 无 dsh-desktop-）——任一条不成立就不武装；因此页面里分不出是哪一类壳也没关系，桌面保护由运行中横幅承担。桌面端退不掉时先关这一项（立即恢复，不用重启）。',
}
...
export const en = {
  ...
  'options.closeGuardDesc': 'On (default): confirm before the page closes — when sessions have running or unfinished work a strong guard lists them (prevents accidental progress loss); otherwise a light one-time confirm. Armed only where the host reports a native confirm dialog (ordinary browser tabs; turning this off also disables the running banner). Desktop (DSH Desktop) never arms it: quit entries differ by version/platform — the 0.9.0-class entry in the external report does reach page beforeunload, and the shell has no will-prevent-unload handler, so the veto is swallowed silently and the exit hangs; the 2.0.9 tray item goes requestQuit→window.destroy→app.exit and never reaches it. Two criteria must both hold: the host says a native dialog appears (quitVeto) AND this page shows no desktop evidence itself (no Electron in the UA, no dsh-desktop- in the URL); if either fails it stays unarmed, so it does not matter that the page cannot tell which shell it is — the running banner carries that protection instead. If Desktop cannot quit, turn this off first (recovers immediately, no restart).',
}
...
//  - 桌面端的保护 = 运行中横幅(有任务即常驻)+ dispose 提示,两者都与退出路径无关;
...
function installCloseGuard(t) {
  ...
  function bannerNote(lang) {
    return lang === 'zh'
      ? '关闭页面/退出前请先确认——任务中断可能丢失进度(桌面端不武装原生退出确认,请以此横幅为准)'
      : 'Confirm before closing/exiting — interrupting work may lose progress (no native exit prompt on Desktop; rely on this banner)'
  }
...
export function apply(ctx) {
  ...
  // 确认,运行中状态经 /runningState 轮询缓存)+ 运行中横幅。原生门只在**宿主回报
  // 本页承载 quit-veto 且客户端不否决**时武装(见 installCloseGuard 段头注释);
  // 桌面端的可见提示 = 运行中横幅 + dispose 提示(与退出路径无关)。
