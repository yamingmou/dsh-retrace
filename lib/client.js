...
/**
 * 规范短码缓存（客户端**同步渲染路径**的唯一来源）。
 *
 * 为什么客户端不能自己算:客户端侧只有一个 FNV 兜底模块 `badge.js`,那套值与宿主
 * 的规范码（表 + 解析器）**不是同一个值** —— 横幅用 FNV、读档点视图用宿主 op,
 * 同一会话就会显示两个码（实测：横幅一套值、读档点视图另一套）。
 * 这里由宿主 op `badgeMap` 一次下发填充;取不到返回空串（调用方显示占位）,
 * **绝不回落 FNV**。
 */
const badgeBySession = new Map()
const normalizeBadgeKey = (id) => String(id ?? '').replace(/^session-/, '').toLowerCase()
...
/** 批量填充（宿主 badgeMap 载荷形状:`{ <verbatim id>: <短码> }`）;返回新增条数。 */
...
/**
 * 短码写入 / 批量 pin 的失败要**显式报出来**（发布前审计 §B）。
 *
 * 静默吞错是这一族问题（"冷列表里既没短码也没名字"）的共同形态：用户看到的是
 * "什么都没发生"，宿主日志里也是 0 命中，无法定位。
 *
 * 两条出口都走：① 渲染器 `console.warn`（人当场可见）；② `clientReport`
 * （客户端侧进入宿主日志的唯一通道，`lib/http.js` 原样打印 `source`）。
 * 诊断只记录，不抛回调用路径。
 */
...
/**
 * 短码诊断（**成功路径**也要留痕,按组合去重）:只有失败才报的话,"发了没到"
 * 与"到了没写"在宿主日志里长得一模一样(都是 0 行)。
 */
const badgeInfoSent = new Set()
 ...
 * 定时器的**安全创建**。
 *
 * ⚠️ 动态客户端 realm 把裸 `setTimeout/setInterval` 换成了会抛错的"教学陷阱"
 * （`dsh-cordis-client-runner` 的 closureTraps，见本文件 scheduleClientReport 的注释）。
 * 裸用会让**整个 apply 抛错** ⇒ 插件 UI 完全不注册（inject=[] definitions=0 slots=0 seats=0）。
 * 2026-09-19 真回归就是这条：一个**可选**的短码数据拉取把整段初始化带崩。
 * ⇒ 一律走 `window.*`；无 window（单测/无 DOM）退到微任务。
 * @returns {() => void} 取消（安全，不抛）
 */
function safeSchedule(fn, delayMs) {
  try {
    ...
    if (typeof schedule === 'function') {
      ...
      return () => { try { window.clearTimeout(handle) } catch { /* ignore */ } }
    }
    if (typeof queueMicrotask === 'function') { queueMicrotask(fn); return () => {} }
  } catch { /* 任何调度失败都不许影响调用方 */ }
  return () => {}
}

/**
 * 冷启动批量 pin 短码标题（发布前审计 §B ①）。
 *
 * 为何由客户端发起：宿主启动时的 `bootPin(0)` 只覆盖"启动那一刻已驻留"的会话
 * （20×30s 窗口）；窗口过后才驻留 / 当时未驻留的会话再没有机会 —— 用户看到的
 * 是"列表里没有短码、名字也没有"。客户端在插件装载时补一次请求（幂等：宿主对
 * 已含短码的会话直接跳过）。
 *
 * 失败处理：不静默 —— 每次失败都 reportBadgeFailure，并退避重试（宿主路由可能
 * 还没就绪）；重试耗尽后留最后一条报告，不再继续。
 *
 * @returns {() => void} 取消挂起的重试（插件卸载时调用）
 */
function bootstrapBadgeTitles({ attempts = 5, delayMs = 3000 } = {}) {
  // 2026-09-20：真机上三次不同失败（not defined / not ready / HTTP 404 + 注册归零）
  // ⇒ 按 §0.3「文件/脚本优先」原则，短码/名字改走脚本层；此处**默认关闭**（置 true 可开）。
  if (globalThis.__DSH_RETRACE_BADGE_BOOTSTRAP !== true) return () => {}

  // 一次性取全量映射（横幅同步渲染路径要用）。失败走 reportBadgeFailure,
  // 不影响标题 pin 的重试链。
  let mapPending
  try { mapPending = callOp('badgeMap', {}) } catch (cause) { mapPending = Promise.reject(cause) }
  ...
  const attempt = () => {
    if (cancelled) return
    let pending
    try { pending = callOp('initBadgeTitles', {}) } catch (cause) { pending = Promise.reject(cause) }
...
function RetraceView({ sessionId, useChat, useProjection, t, actions, store }) {
  ...
  useEffect(() => {
    ...
    // 写入短码 title（一次：如果标题已含短码则跳过）。失败**不静默**：
    // host 返回 ok:false 或请求抛错都要报出来（发布前审计 §B）。
    ...
    // 语义短码（2026-09-01）：host 短码表优先（工作区+序号）；表里没有的会话
    // 由 host 的解析器按"钉住 + 追加"给出，不再回退 FNV。
...
//
// 事实与选型(2026-09-18 外部 issue #1 复核后重写;详见 lib/close-guard-client.js
// 头注释与 lib/close-guard.js 头注释):
//  - 退出入口随 Desktop 版本/平台而变:外部报告的 0.9.0 / Windows 那一类入口会触发页面
//    beforeunload,而 DSH Desktop 的 Electron 壳没有处理 will-prevent-unload
//    (装好的 2.0.9 app.asar 全文 0 命中)⇒ 否决被静默吞掉,那里表现为"退出卡住、
//    只能强退";2.0.9 的托盘项走 requestQuit(0) → window.destroy() → app.exit(code),
//    不经过 beforeunload,那类版本上退出本来就正常;
//  - ⇒ 原生门的武装条件 = **宿主明确回报本页承载 quit-veto**(普通浏览器页面)
//    **且本页自己看不出桌面痕迹**(UA 无 Electron、URL 无 dsh-desktop-;2026-09-18
//    第二轮补的客户端一票否决)。承载面由宿主随 runningState 载荷下发
//    (quitVeto === true 且客户端不否决才武装;false/未知都不武装,未知取安全侧);
//  - 桌面端不武装的只是**宿主原生确认框**那条路(壳没处理 will-prevent-unload),
//    改用**页面自绘确认框**:先画框并确认看得见,才拦下关闭;框画不出来、不可见或
//    页面不可见 ⇒ 当场放行(Web Worker 计时器只看门,不受后台节流);2.0.9 托盘/⌘Q
//    不经过 beforeunload 的路径仍由运行中横幅 + dispose 提示覆盖;
//  - 浏览器 beforeunload 确认框文案/按钮不可自定义(Chromium 统一原生框),
//    故浏览器 A 强拦 = 原生门 + 「取消后」的中文明细模态;B 轻确认 = 原生一次确认。
// ---------------------------------------------------------------------------
...
function installCloseGuard(t) {
  ...
  /**
   * 会话 id → 显示名。码一律取**规范短码缓存**（宿主 badgeMap 下发）;
   * 未知时退回原始 id 占位（用户明确"原始 id 显示与否不作要求"）——
   * **绝不**回落本地 FNV:那正是"横幅与读档点视图两个码"的来源。
   */
  ...
  function refresh() {
    ...
    queryRunningState().then((payload) => {
      ...
      // quitVeto = 宿主回报的"本页是否承载 quit-veto"(见 close-guard-client.js
      // shouldArmNativeGate:宿主值 + 客户端一票否决两道)。**原样缓存宿主的值**:
      // 缺字段 = 旧宿主 = 未知 ⇒ 不武装。
  ...
  function bannerNote(lang) {
    return lang === 'zh'
      ? '关闭页面/退出前请先确认——任务中断可能丢失进度(桌面端用页面自绘确认框;若未弹出,以此横幅为准)'
      : 'Confirm before closing/exiting — interrupting work may lose progress (Desktop uses a page-drawn confirm dialog; if it does not appear, rely on this banner)'
  }
  ...
  // -- 页面关闭拦截(beforeunload;只在宿主确认承载 quit-veto 的页面上武装) ----
  function onBeforeUnload(event) {
    ...
    // 宿主承载面闸门(2026-09-18 外部 issue #1):桌面 Electron 页面上原生
    // beforeunload 门没有意义(2.0.9 那类入口不经过它;0.9.0 那类入口经过时否决被
    // 壳静默吞掉、退出卡住)。**两道判据都为真才武装**:
    //   ① 宿主明确回报 quitVeto === true(它认为这是会弹原生确认框的普通浏览器页);
    //   ② 页面自己**没有**桌面证据(navigator.userAgent 无 Electron 段、URL 无
    //      `dsh-desktop-` 标记)—— 2026-09-18 第二轮补的**客户端一票否决**:
    //      报告人那台壳三条宿主判据全不成立 ⇒ 宿主错回 browser/true,只靠 ① 会照旧
    //      卡死(实测)。false 与未知一律不武装(未知取安全侧)。
...
export function apply(ctx) {
  ...
  // 冷启动批量 pin 短码标题：客户端侧唯一的启动触发点。挂在 effect 上是为插件
  // 卸载时清掉未完成的重试定时器。失败走 reportBadgeFailure（不静默）。
  // ⚠️ 这是**可选**的数据拉取：任何同步异常都吞在这里并返回空 disposer，
  // 让后面的 slot/Definition 注册照常执行（2026-09-19 真回归的修复点）。
  ctx.effect(() => {
    try { return bootstrapBadgeTitles() } catch (error) {
  ...
  // 确认,运行中状态经 /runningState 轮询缓存)+ 运行中横幅。**宿主原生确认框**那条
  // 路只在宿主回报本页承载 quit-veto 且客户端不否决时武装;桌面端不武装它,改用
  // **页面自绘确认门**(见 close-guard-client.js:先画框并验证可见才拦,失败或不可见
  // 时当场放行;worker 计时器只看门),2.0.9 托盘/⌘Q 路径由运行中横幅 + dispose 提示覆盖。
