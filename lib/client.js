...
export const zh = {
  ...
  // issue-199/200:不再原样透传 host 错误(曾为英文 "no longer part of the active
  ...
  // issue-229:新增两个 host 错误码(message-not-found / span-replay-failed)+ 契约违规走通用文案
...
export const en = {
  ...
  // issue-199/200:localized by code (see zh). Never show the raw host copy verbatim.
 ...
 * 兜底 error.generic。host 同时在 error 上附带 messageId/seq(排查用,issue-200 建议 3)。
 */
export function opFailureText(code, message, t) {
  ...
  // issue-229:显式状态落到的新错误码(缺了会让英文用户看到 host 中文文案)
...
const CONFIG_DEFAULTS = { version: CONFIG_VERSION, showOriginalInput: true, editFromScratch: false, hideShadowed: true, versioning: true, git: true, retentionLimit: 50, prewrite: true, closeGuard: true }
...
function RetraceView({ sessionId, useProjection, t, actions, store }) {
  ...
  // 不用 FNV 兜底显示——维护线规则是 member-65member-16 这类语义码,不是随机 hash。
...
function ForkView({ sessionId, useProjection, t, actions, store }) {
  ...
  // Renderer CPU 27.7%，5e551007 打开卡死）。其他会话节点少不触发，仅大会话暴露。
  const ROW_H = 56
  const visibleStart = Math.max(0, Math.floor(scrollTop / ROW_H) - 2)
...
// ---------------------------------------------------------------------------
// 关闭守卫 V2 (issue-176, 2026-09-09) — 装配层(纯逻辑/文案在
...
export function apply(ctx) {
  ...
  // 关闭守卫 V2(issue-176,2026-09-09):页面关闭 beforeunload 拦截(A 强拦/B 轻
