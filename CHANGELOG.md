...
- **默认配置由破坏性改为安全**:`editFromScratch` 默认 `true → false`(编辑一条消息不再回绕隐藏整个编辑点之前的会话)、`hideShadowed` 默认 `true → false`(被编辑/撤回的消息默认保持可见,标记仅显示提示与对照)。配置带版本号,v1 → v2 自动迁移,仅重置这两个键,其余自定义保留。
