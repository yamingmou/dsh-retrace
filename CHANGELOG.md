...
## [0.4.32] — 2026-09-20 · 文档修复：README 随版本更新（+ 文案锁同步）

- **修正两份 README（中/英）**：关闭拦截改为**准确口径** ——「**不武装宿主原生确认框**；改用**页面自绘确认门**：先画框 + 同步可见校验才拦，画不出/不可见/页面不可见 ⇒ 当场放行；Esc=取消；Web Worker 看门狗只作最后一道；关设置项即退回官方形态」；**新增「已知限制」**（某些壳的退出口径不经页面 ⇒ 插件侧做不出"退出弹确认"，需壳 seam）；**新增「会话短码与显示名」**（`[短码] 原标题`、`opxxxopxxx`、真表解析、无 title 如实留空、拿不到表退原始 id、**绝不回落 FNV**）。
- **文案锁同步加强**：把已变错的两句换成更正措辞，并新增 6 条事实锁（自绘门 / 壳 seam / 短码形态，中英各一）。
- **说明**：`0.4.31` 已撤回（npm 上标记 **deprecated**、公开仓 tag 已删）—— 其 README 未随版本更新。

## [0.4.31] — 2026-09-19 · 展示层(短码/名字) + 桌面关闭拦截(自绘确认门)

### 修复/新增
- **短码与名字**：会话标题按「`[短码] 原标题`」写入（官方 rename 通道），侧栏随标题显示；短码用真表（111 条）解析，**拿不到就退回原始 id 占位，绝不回落 FNV**；根因修正 = `ctx.sessions.list()` 只含**活会话**（启动时为空 ⇒ 旧逻辑迭代 0 次），改为 `session/created` 事件补 pin。
- **桌面关闭拦截（重做）**：**不再用阻塞式原生门**（桌面壳无 `will-prevent-unload` 处理器 ⇒ 静默否决 = 退出卡死）。改为「拦下 + **页面自绘确认框** + 确认后放行」，并先挂 **1500ms 看门狗**：确认框未渲染/尺寸 0/挂载抛错 ⇒ **无条件放行**。`[取消]` 留页，`[仍要关闭]` 真退。
- **可退**：设置项「退出确认（关闭守卫）」关掉即停（免重启）。
- **已知限制（照实）**：本机 2.x 壳的 **⌘Q/托盘退出不经过 `beforeunload`**（`close` 只是隐藏、`requestQuit` 走 `destroy+app.exit`、宿主在隔离 utility 进程）⇒ 该入口插件侧做不出确认框，需官方壳 seam。真正覆盖 = 会走到 `beforeunload` 的入口（0.4.28/0.9.0 一类壳、浏览器页、渲染器重载）。

## [0.4.30] — 2026-09-19 · 桌面端退出仍卡死（issue #1 第二轮）+ 承载面探针

### 修复
- **判据补成正向证据，"完全没有证据"不再被归成浏览器页**：0.4.29 把桌面判定交给宿主，
  但只在"挂能力头 / 有桌面服务 / 进程是 Electron"三条判据里认。报告人那台（纯 Node 跑
  harness + Electron 渲染页）**三条一条都不成立** ⇒ 被归成普通浏览器页 ⇒ 客户端照旧武装
  原生门 ⇒ **退出仍然卡死**（他抓到的原始回包：`{"running":[],"surface":"browser","quitVeto":true}`）。
  现补两条**正向**判据：请求 `User-Agent` 含 `Electron`、请求 URL 含 `dsh-desktop-`
  （报告人那套壳的查询参数前缀）；任一成立即判桌面页面。
- **客户端一票否决**：页面自己的 `navigator.userAgent` 含 Electron 或页面 URL 含
  `dsh-desktop-` ⇒ **一律不武装**，不管宿主怎么说。语义 = 宿主说"可以武装"只是**不反对**，
  两者都为真才武装（宿主判错也卡不住退出）。
- **承载面探针**：把每次判定的入参（能力头 / 请求 UA / 请求 URL 标记 / 宿主桌面痕迹）与
  结果（`surface` / `quitVeto`）**按入参组合去重**写进宿主日志（组合空间 4 个布尔 ⇒ 最多 16 种，
  结构上不会刷屏）。报告人跑一次就能确认是哪条判据生效，不必再互相猜。
- **设置项文案 + README（中/英）**：写明两道判据，并给出自救开关
  "**桌面端退不掉时先关这一项**（立即恢复，不用重启）"。

### 测试
- 新增锁：报告人现场回归（宿主错回 `browser/true` + 页面 UA 含 Electron ⇒ 不 preventDefault）、
  请求 UA / 请求 URL 两条宿主判据、普通浏览器页正向对照（Web 保护不变）、探针去重、
  文本与源码出口锁。
- 变异锁 6 项**该红时红**且源文件逐字节还原（删 UA 判据 / 删 URL 判据 / 去掉探针去重 /
  一票否决失效 / 页面证据退化为"永远不是桌面" / 装配层回到裸 `quitVetoOf`）。
- 全量 **1190 passed**；派生件已重建。

## [0.4.29] — 2026-09-18 · 修复桌面端退出死锁（外部 issue #1）+ 定时器与对外文本

### 修复
- **桌面端不再武装原生 `beforeunload` 门**：DSH Desktop 的 Electron 壳不处理 `will-prevent-unload`，
  页面否决在桌面端是**静默**的；若退出入口会经过 `beforeunload`（不同桌面项目/版本实现不同，
  已知至少两种：`app.quit()` 与 `requestQuit → window.destroy() → app.exit()`），
  页面否决就会让**退出静默卡住、只能强退**。现在桌面端**一律不武装**，保护由运行中横幅 + dispose 提示承担。
- **承载面判定由宿主给出**：`runningState` 载荷新增 `surface` / `quitVeto`；客户端**只在
  `quitVeto === true`**（宿主确认该页面会弹原生确认框，即普通浏览器页）时武装；`false` 与"未知"一律不武装。
- **两个定时器**：`bootPin` 的 `setTimeout` 句柄此前被丢弃、无 `unref`、未纳入 `disposers`；
  `watchdog` 的 `setInterval` 是引用型 —— 两者都会拖住宿主进程退出。现已 `unref` 并纳入清理路径。
- **对外文本与事实对齐**：设置项文案、README（中/英）、模块注释统一改为"退出入口随版本/平台而变"，
  不再出现"桌面端都会/都不会触发"这类**把未覆盖路径当结论**的表述。

### 测试
- 新增/加固：桌面不武装、Web 仍武装、承载面判定、定时器生命周期、对外文本的"版本限定"锁定。
- 全量 **1159 passed**；派生件已重建。
